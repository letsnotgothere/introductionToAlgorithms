...
class RangeIndex(object):
	...
	def remove(self, key, value):
		...
		self.data.delete(key, value)

	def list(self, first_key, first_key_value, last_key, last_key_value):
		"""List of values for the keys that fall within [first_key, last_key]."""
		return self.data.list(first_key, last_key)

	def count(self, first_key, first_key_value, last_key, last_key_value):
		"""Number of keys that fall within [first_key, last_key]."""
		#print("Now counting between " + str(first_key) + " and " + str(last_key))
		min = self.data.min()
		#print("mininmum key currently " + str(min.key))
		first_exists = self.data.find(first_key) is not None
		#if first_exists:
			#print("Key " + str(first_key) + " exists in tree")
		#else:
			#print("Key " + str(first_key) + " does not exist in tree")
		first_rank = self.data.rank(first_key)
		#print("rank of first key: " + str(first_rank))
		last_rank = self.data.rank(last_key)
		#print("rank of last key: " + str(last_rank))
		if first_exists:
			count = last_rank - first_rank + 1
		else:
			count = last_rank - first_rank
		return count
...
class TracedRangeIndex(RangeIndex):
	...
	def add(self, key, value):
		self.trace.append({'type': 'add', 'id': value.wire.name})
		RangeIndex.add(self, key, value)

	def remove(self, key, value):
		self.trace.append({'type': 'delete', 'id': value.wire.name})
		RangeIndex.remove(self, key, value)

	def list(self, first_key, first_key_value, last_key, last_key_value):
		result = RangeIndex.list(self, first_key, first_key_value, last_key, last_key_value)

		results = []
		for node in result:
			for kwp in node.value:
				results.append(kwp)

		self.trace.append({'type': 'list', 'from': first_key,'to': last_key,'ids': [kwp.wire.name for kwp in results]})
		return result

	def count(self, first_key, first_key_value, last_key, last_key_value):
		result = RangeIndex.count(self, first_key, first_key_value, last_key, last_key_value)
		self.trace.append({'type': 'list', 'from': first_key,'to': last_key, 'count': result})
		return result
...
class CrossVerifier(object):
	...
	def _compute_crossings(self, count_only):
		...
		for event in self.events:
			...
			if event_type == 'add':
				self.index.add(wire.y1, KeyWirePair(wire.y1, wire))
			elif event_type == 'remove':
				self.index.remove(wire.y1, KeyWirePair(wire.y1, wire))
			elif event_type == 'query':
				'''
				if wire.name == 'left-0-right':
					print(wire.__dict__)
					print("")
					rs = self.index.list(-200,None,200,None)
					for r in rs:
						print_tree(r)

					c = self.index.count(wire.y1, None, wire.y2, None)
					print(c)
				'''
				if count_only:
					result += self.index.count(wire.y1, KeyWirePairL(wire.y1), wire.y2, KeyWirePairH(wire.y2))
					#print("adding " + str(self.index.count(wire.y1, KeyWirePairL(wire.y1), wire.y2, KeyWirePairH(wire.y2))) + " to total")
				else:
					for node in self.index.list(wire.y1, KeyWirePairL(wire.y1), wire.y2, KeyWirePairH(wire.y2)):
						for kwp in node.value:
