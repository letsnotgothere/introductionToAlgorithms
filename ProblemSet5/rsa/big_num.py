...
class BigNum(object):
	...
	def slow_mul(self, other):
		...
		C = BigNum.zero(len(self.d) + len(other.d))

		for i in xrange(0,len(self.d)):
			#print("I = " + str(i))
			#print("Resetting carry")
			carry = Byte.zero()
			for j in xrange(0,len(other.d)):
				#print("J = " + str(j))
				digit = (self.d[i] * other.d[j]) + C.d[i+j].word() + carry.word()
				#print("Inserting " + str(digit.lsb()) + " into C[" + str(i+j) + "]")
				C.d[i+j] = digit.lsb()
				#print("Carrying " + str(digit.msb()))
				carry = digit.msb()
				#print(C.__dict__)
			#print("Inserting (carried)" + str(carry) + " into C[" + str(i + len(self.d)) + "]")
			C.d[i+len(other.d)] = carry
			#print(C.__dict__)
		return C.normalize()
